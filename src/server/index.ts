...
export function createVorzelaApp(options: CreateAppOptions) {
  ...
  if (isProd && staticRoot) {
    ...
    // PWA files (generated by vorzelajs-pwa plugin)
    app.get('/sw.js', async (c) => {
      const filePath = path.resolve('./dist/client/sw.js')
      try {
    ...
    app.get('/manifest.webmanifest', async (c) => {
      const filePath = path.resolve('./dist/client/manifest.webmanifest')
      try {
        const content = await fs.promises.readFile(filePath, 'utf-8')
    ...
    app.get('/offline.html', async (c) => {
      const filePath = path.resolve('./dist/client/offline.html')
      try {
        const content = await fs.promises.readFile(filePath, 'utf-8')
