...
function formatRelativeId(id: string, root: string) {
...
function getServerOnlyImportInfo(
  ...
  const throwClientBoundaryError = (specifier: string, reason: string) => {
    throw new Error(
      `[vorzelajs-server-only] ${formatRelativeId(id, root)} ${reason} "${specifier}". `
...
function stripRouteServerProperties(code: string, id: string, root: string) {
...
function assertNoServerOnlyImportsInClientFile(code: string, id: string, sourceRoot: string, root: string) {
  const normalizedId = stripQuerySuffix(id)
  ...
  const throwClientBoundaryError = (specifier: string) => {
    throw new Error(
      `[vorzelajs-server-only] ${formatRelativeId(id, root)} imports .server module "${specifier}". `
...
export function vorzelaServerOnlyPlugin(): Plugin {
